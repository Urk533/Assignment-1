...
?>
		...
		<!-- Below h1 heading contains the value which defines the lists available in this webpage -->
			...
			<!-- From below php code starts where code for listing latest auctions are written -->
			<?php

			//latestAucStmt variable is used to start the select query for selecting the auctions from auction table in database
			//which is order by auction id in descending order so that latest article will be displayed
		    $latestAucStmt = $pdo->prepare('SELECT * FROM auction ORDER BY auctionId DESC LIMIT 10');
			
			//Below code helps to exexute above selectquery
			$latestAucStmt->execute();
		
			//latestFetchedAucVal stores all the fetched values of the table of latestAucStmt variable
			$latestFetchedAuc = $latestAucStmt->fetchAll();

			//Below if statements check whether the latestFetchedAuc contains the values of the column of auction table or not
			if($latestFetchedAuc){?>
			...
			<ul class="productList">
				...
				foreach($latestFetchedAuc as $rows){
